...
def obstacle_thresh(img, rgb_thresh = (160, 160, 160)):
    ...
    below_thresh = (img[:,:,0] <= rgb_thresh[0]) \
                | (img[:,:,1] <= rgb_thresh[1]) \
                | (img[:,:,2] <= rgb_thresh[2])
    obstacles[below_thresh] = 255
    return obstacles

def rock_thresh(img, rgb_thresh_upper = (350, 170, 60), rgb_thresh_lower = (0, 80, 0)):
    rocks = np.zeros_like(img[:,:,0])
    in_thresh = ((img[:,:,0] >= rgb_thresh_lower[0]) & (img[:,:,0] <= rgb_thresh_upper[0]) \
...
def perception_step(Rover):
    ...
    xpos, ypos = Rover.pos
    world_x, world_y = pix_to_world(x_pixel, y_pixel, xpos, ypos, Rover.yaw, 200, 10)
    rock_x_world, rock_y_world = pix_to_world(rock_x, rock_y, xpos, ypos, Rover.yaw, 200, 10)
    obstacle_x_world, obstacle_y_world = pix_to_world(obs_x, obs_y, xpos, ypos, Rover.yaw, 200, 10)
    ...
    if (Rover.pitch <= 1 or Rover.pitch >= 359) and (Rover.roll <= 1 or Rover.roll >= 359):
        ...
        Rover.worldmap[rock_y_world, rock_x_world, 1] += 1
        Rover.worldmap[world_y, world_x, 2] += 1
